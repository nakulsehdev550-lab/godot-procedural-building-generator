...
static func _build_spiral(st: SurfaceTool, st_trim: SurfaceTool, plan_d: Dictionary,
        ...
        for i in steps:
                ...
                # orbiting the POLE (cell center), not the cell corner
                var mid := (a0 + a1) * 0.5
                var span := (a1 - a0) * (r_out + r_in) * 0.5
                var step_xf := _rotated_step(Transform3D(Basis.IDENTITY,
                        Vector3(c_local.x, y0 + riser * 0.5 + 0.02, c_local.z)), mid, (r_in + r_out) * 0.5)
...
static func _rotated_step(xfm: Transform3D, mid: float, rmid: float) -> Transform3D:
        var pos := Vector3(xfm.origin.x + cos(mid) * rmid, xfm.origin.y, xfm.origin.z + sin(mid) * rmid)
