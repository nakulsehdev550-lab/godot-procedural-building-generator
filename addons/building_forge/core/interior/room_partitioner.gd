...
static func partition(bounds: Rect2, stair_cell: Rect2, max_room_area: float, rng: RandomNumberGenerator) -> Dictionary:
	var rooms: Array = []
	var queue: Array = [bounds.grow(-0.05)]
	var next_id := 0
	# carve out stairwell: split any room overlapping the cell
	var guard := 0
	while queue.is_empty() == false and guard < 4096:
		guard += 1
		var r: Rect2 = queue.pop_back()
		var clipped := r.intersection(stair_cell)
		if clipped.get_area() > 0.3:
			# split around the stair cell (axis-aligned guillotine cuts)
